...
//        player.save();//不保存，待记录员点击提交按钮后  再保存，否则该次修改无效
    ...
    void doModifyMatch() {
        ...
        if (mDetailMatch.playerNumber == 0) {
            ToastUtil.showToast(getApplicationContext(), "请设置赛制人数");
            return;
        }

        if (mDetailMatch.totalPart == 0) {
            ToastUtil.showToast(getApplicationContext(), "请设置比赛节数");
            return;
        }

        if (mDetailMatch.partMinutes == 0) {
            ToastUtil.showToast(getApplicationContext(), "请设置每节时长");
            return;
        }
